...
function ImprovedTable($header)
{
    ...
    $w = array(40, 20, 25, 16, 26, 25, 18, 25, 25, 25);
    // Cabeceras
...
for($i=0;$i<sizeof($user);$i++){
     $pdf->Cell(40,8,strtoupper($user[$i]['nombre_user']),0,0,'C');
     ...
     $pdf->Cell(26,8,strtoupper($sex),0,0,'C');
     ...
     $pdf->Cell(18,8,strtoupper($tv),0,0,'C');
                                     $discap=new discap();                                                    
                                     $dis=$discap->leer_discap_user($user[$i]['id_user']);  
                                     $pos=$dis[0]['disc_user'];
                                     
                                     if($pos==0){
                                   $pdf->Cell(25,8,"NO APLICA",0,0,'C');
                                     }else{
                                     $discap=new discap();
                                     $dis=$discap->leer_discap_id($pos); 
                                    $pdf->Cell(25,8,  strtoupper($dis[0]['discapacida']),0,0,'C');
                                     }
     ...
     $indigena=new indigena();                                                    
                                 ...
                                 $pdf->Cell(25,8,strtoupper($ind[0]['comunidad']),0,0,'C');
                                     }
